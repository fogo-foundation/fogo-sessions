...
export const useAddressLookupTable = (
  ...
  useEffect(() => {
    if (!addressLookupTableAddress) {
      ...
      connection
        .getAddressLookupTable(new PublicKey(addressLookupTableAddress))
        .then((account) => {
          setState(State.Complete(account.value ?? undefined));
        })
        .catch((error: unknown) => {
...
const State = {
  ...
  Complete: (addressLookupTable: AddressLookupTableAccount | undefined) => ({
    type: StateType.Complete as const,
    addressLookupTable,
  }),
