...
>(
  name: string,
  defaults: { localnet: D; testnet: P, mainnet: M },
) => {
  const valueFromEnv = process.env[name];
  if (valueFromEnv === undefined) {
    return IS_DEV_MACHINE && process.env.USE_TESTNET === undefined && process.env.USE_MAINNET === undefined
      ? defaults.localnet
      : process.env.USE_TESTNET
      ? defaults.testnet : defaults.mainnet;
  } else {
    return valueFromEnv;
  }
};
...
export const PAYMASTER = envWithDefaults("PAYMASTER", {
  ...
  testnet: undefined,
  mainnet: "https://paymaster.dourolabs.app",
});
...
export const FOGO_SESSIONS_DOMAIN = envWithDefaults("FOGO_SESSIONS_DOMAIN", {
  ...
  mainnet: "https://sessions-example.fogo.io",
});
