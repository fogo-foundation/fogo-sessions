...
import type { AccountConstraint, ContextualPubkey, DataConstraint, DataConstraintSpecification, InstructionConstraint, PrimitiveDataType, PrimitiveDataValue, Variation } from '../../../db-schema';
...
function convertVariationToToml(variation: Variation) {
  if (variation.version === "v0") {
    ...
    ` + [variation.transaction_variation.map(ix => convertInstructionConstraintToToml(ix)).join("\n")];
  } else {
    throw new Error(`Unknown variation version`);
  }
}

function convertInstructionConstraintToToml(instruction: InstructionConstraint) {
    return `
      [[domains.tx_variations.instructions]]
      program = "${instruction.program}"
      required = ${instruction.required}
    ` + [instruction.accounts.map(acc => convertAccountConstraintToToml(acc)).join("\n")]
      + [instruction.data.map(data => convertDataConstraintToToml(data)).join("\n")];
}
...
function convertDataTypeToToml(dataType: PrimitiveDataType) {
  if (typeof dataType === 'string') {
    ...
    throw new Error(`Unknown PrimitiveDataType variant: ${JSON.stringify(dataType)}`);
  }
}

function convertDataConstraintSpecificationToToml(constraint: DataConstraintSpecification) {
  if ('LessThan' in constraint) {
    ...
    throw new Error(`Unknown DataConstraintSpecification variant: ${JSON.stringify(constraint)}`);
  }
}
...
function convertPrimitiveDataValueToToml(value: PrimitiveDataValue) {
  if ('U8' in value) {
    ...
    throw new Error(`Unknown PrimitiveDataValue variant: ${JSON.stringify(value)}`);
  }
}
