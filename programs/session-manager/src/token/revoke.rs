...
use fogo_sessions_sdk::session::{AuthorizedTokens, AuthorizedTokensWithMints, SessionInfo, V2, V3};
...
impl<'info> CloseSession<'info> {
    ...
    pub fn revoke_tokens(
        ...
        let (user,mints_to_revoke) = match &self.session.session_info {
            SessionInfo::V3(V3::Revoked(revoked_info)) => {
                match &revoked_info.authorized_tokens {
            ...
            SessionInfo::V2(V2::Active(active_session_info)) => {
                match active_session_info.authorized_tokens {
                    AuthorizedTokens::Specific => return Err(error!(SessionManagerError::InvalidVersion)),
                    AuthorizedTokens::All => (&active_session_info.user, &vec![]),
                }
            },
            SessionInfo::V1(v1) => {
                match v1.authorized_tokens {
                    AuthorizedTokens::Specific => return Err(error!(SessionManagerError::InvalidVersion)),
        ...
        require_eq!(mints_to_revoke.len(), accounts.len(), SessionManagerError::MissingAccount);
        mints_to_revoke.iter().zip(accounts.iter()).map(|(mint, user_account)| {
            require_eq!(
                user_account.key(),
            ...
            if user_account.owner == &self.token_program.key() {
                let account_data = TokenAccount::try_deserialize(&mut user_account.data.borrow().as_ref())?;
