...
impl SendTokenFeeConfig {
    pub fn verify_and_collect_ata_fee(&self, args: VerifyAndCollectArgs, fee_amount: String, fee_symbol_or_mint: SymbolOrMint, signer_seeds: &[&[&[u8]]]) -> Result<()> {
        self.verify_and_collect_fee(self.ata_creation_fee, args, fee_amount, fee_symbol_or_mint, signer_seeds)
    }

    pub fn verify_and_collect_bridging_out_fee(&self, args: VerifyAndCollectArgs, fee_amount: String, fee_symbol_or_mint: SymbolOrMint, signer_seeds: &[&[&[u8]]]) -> Result<()> {
        self.verify_and_collect_fee(self.bridging_out_fee, args, fee_amount, fee_symbol_or_mint, signer_seeds)
    }

    fn verify_and_collect_fee(&self, fee: u64, args: VerifyAndCollectArgs, fee_amount: String, fee_symbol_or_mint: SymbolOrMint, signer_seeds: &[&[&[u8]]]) -> Result<()> {
        let VerifyAndCollectArgs {
            fee_source, fee_destination, fee_mint, fee_metadata, intent_transfer_setter, token_program } = args;
