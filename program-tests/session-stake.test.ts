...
import type { Snapshot, TransactionMetadata } from "@xlabs-xyz/fork-svm";
import { ForkSvm } from "@xlabs-xyz/fork-svm";
import { type Ix, stakeProgramId, minimumBalanceForRentExemption } from "@xlabs-xyz/svm";
import {
  fogoRpcUrl,
  fogo,
  assertTxSuccess,
...
type TestContext = {
  ...
  send: (
      ixs: RoArray<Ix>,
      extraSigners?: RoArray<KeyPairSigner>,
    ) => Promise<TransactionMetadata>;
  initAndDeposit: (amount: bigint) => Promise<Address>;
};
...
describe("SessionStake", function() {
  ...
  const getFogoBalance = async <const A extends MaybeArray<Address>>(addressEs: A) =>
    mapTo(await forkSvm.getAccount(addressEs))(accInfo => accInfo?.lamports ?? 0n);
  ...
  const createActiveStakeAccount = (
    ...
    createAccount(pubkey, new Uint8Array(data), stakeProgramId, rentExempt + stakedLamports);
  };
  ...
  before(async () => {
    ...
    userFogo = createSplFogoAta(user, initialUserFogo);
    ...
    createAccount(domainRecordPda(testDomain), domainRecord, domainRegistryProgramId);
  ...
  const sharedTests = (getCtx: () => TestContext) => {
    ...
    test("deposit and withdraw", async () => {
      const ctx = getCtx();
      const { user, signerOrSession, payer, userFogo } = ctx;
      const depositAmount  = fogo(5);
    ...
    test("move stake between active accounts", async () => {
      ...
      const getStakedAmount = async (addr: Address) => {
        ...
        if (state?.__kind !== "Stake") throw new Error("Expected Stake state");
  ...
  const makeCtx = (
    ...
    const initAndDeposit = async (amount: bigint) => {
      ...
      await send([
        ...
        composeDepositIx(amount as Lamports, {
          user,
          signerOrSession,
          payer,
          stake,
    ...
    return { user, signerOrSession, payer, userFogo, send, initAndDeposit };
  };
