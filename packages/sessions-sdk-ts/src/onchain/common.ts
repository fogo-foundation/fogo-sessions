...
export const pdaOfProgram =
  (programId: Address) =>
    <const S extends Parameters<typeof findPda>[0]>(seeds: S) =>
      findPda(seeds, programId);

export const u64Item = { binary: "uint", size: 8, endianness: "little" } as const;

export const nonceLayout =
  accountLayout("Nonce", u64Item);
...
export const byteDiscriminatedLayout = <const L extends Layout>(
  ...
  ? layout.length ===  0
    ...
    : byteDiscriminatedProperLayout(discriminator, layout as ProperLayout)
  : byteDiscriminatedItem(discriminator, layout as Item)
) as any;
