...
export function createSessionWrapInstruction (sessionKey: PublicKey, walletPublicKey: PublicKey, amount : bigint) {
...
export function createSessionWrapInstructions(sessionKey: PublicKey, walletPublicKey: PublicKey, amount : bigint) {
    const userTokenAccount = getAssociatedTokenAddressSync(NATIVE_MINT, walletPublicKey);

    return [
        createSessionWrapInstruction(sessionKey, walletPublicKey, amount),
        createAssociatedTokenAccountIdempotentInstruction(sessionKey, userTokenAccount, walletPublicKey, NATIVE_MINT),
        createSyncNativeInstruction(userTokenAccount),
    ];
}

export function createSessionUnwrapInstruction(sessionKey: PublicKey, walletPublicKey: PublicKey) {
    const userTokenAccount = getAssociatedTokenAddressSync(NATIVE_MINT, walletPublicKey);
    return createCloseAccountInstruction(userTokenAccount, walletPublicKey, sessionKey);
}
