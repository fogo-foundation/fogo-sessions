...
impl Tolls {
    ...
    pub async fn validate_toll_payment(
        &self,
        transaction: &VersionedTransaction,
        chain_index: &ChainIndex,
    ) -> Result<(), (StatusCode, String)> {
        let tollbooth_instructions: Vec<(usize, &CompiledInstruction)> = transaction
            .message
            .instructions()
            .iter()
            .enumerate()
            ...
            (Tolls::Fixed(tolls), [(instruction_index, tollbooth_instruction)]) => {
                ...
                let mint = chain_index
                    .resolve_instruction_account_pubkey(
                        ...
                        *instruction_index,
                        Self::MINT_ACCOUNT_INDEX_PAY_TOLL_INSTRUCTION,
                    )
                    .await?;

                if tolls
                    .iter()
                    .any(|toll| toll.mint == mint && toll.amount == amount)
                {
                    return Ok(());
                }
                return Err((
