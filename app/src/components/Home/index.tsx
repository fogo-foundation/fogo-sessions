...
const handleEnableTrading = async (
  ...
  setLink: (link: string) => void,
    ...
    const response = await fetch('/api/sponsor_and_send', {
      ...
      body:  JSON.stringify({transaction: Buffer.from(transaction.serialize({requireAllSignatures: false})).toString("base64")}),
    })

    const lastValidBlockHeight = await provider.connection.getSlot();
    const { signature } = await response.json()
    const confirmationResult = await provider.connection.confirmTransaction({signature, blockhash: transaction.recentBlockhash, lastValidBlockHeight})
...
export const Home = () => {
  ...
  const onEnableTrading = useCallback(() => {
    if (signMessage && publicKey) {
      handleEnableTrading(sessionManagerProgram, publicKey, signMessage, setLoading, setStatus, setLink).catch(
        (error) => {
          setLoading(false);
          console.error(error);
        },
